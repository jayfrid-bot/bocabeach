...
## Implementation notes (2026-09-22, after two Codex implementation rounds)
- **Free Workers plan = 50 subrequests per request; a cold conditions build is ~25.** So the archive route does **one build per call** and the cron runs **every minute**: each tick scans candidates least-recently-archived first, claims the first free (slug, hour), and builds it — so all 39 beaches get their hourly row and a stuck claim never blocks the queue. The 600/day budget still caps upstream load. Raising the per-call batch requires Workers Paid.
- **Claims + reservations happen before any fetch:** `history_claims` (key `history:<slug>:<hour_utc>`, 10-min abandonment window, `completed_at` on success) and an atomic `history_budget` reservation for the current UTC day (`HISTORY_MAX_BUILDS_PER_DAY`, default 600, leaving ~4k Open-Meteo calls/day for visitors under the free tier). `HISTORY_ENABLED=off` pauses archiving instantly.
- **The claimed hour is the row's hour.** `getConditions` is cached ~2 min, so the route passes the claimed `hour_utc` into `rowFromConditions`; a snapshot older than the hour start by >10 min is skipped and its claim released for the next tick.
