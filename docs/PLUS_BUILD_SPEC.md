...
**One hazard truth (amended 2026-09-18, see `docs/LOCATION_FIRST_PLAN.md` phase 1).** Whether it is raining or lightning is near is decided in exactly one place, `lib/hazards/assess.ts` (`assessLightning`, `assessRain`), and both the score's caps (`lib/score.ts` `applyBeachCaps`) and the at-beach alert subjects consume it, so a push and the displayed number are computed from the same rule. The assessment carries its anchor, the observation it rests on, and a hold: lightning stays active for 30 min after the most recent strike within 5 mi (`closeStrikeMinutesAgo` from `lib/sources/lightning.ts`; the GLM feed window is 30 min, so the hold never outlives the data), rain for 20 min after the radar last saw the beach wet (`lastWetIso`, recorded by the MRMS job from every decoded frame and carried across runs; parsed to `wetMinutesAgo` on the server clock, and preserved through a transient empty publication). The radar dry veto is only "confident" when the frame is fresh, dry now, and not wet within the hold. Anchors are honest: the score uses `beach`; alerts use the device fix (`point`) for lightning, and for rain use the beach radar (`beach`) or, when the radar has nothing, the fix's 0.05° cell forecast (`point`). A latched-only rain state holds the cap and suppresses "clearing" but never writes the rain-wet mark or erases an upcoming rain ETA. The score itself stays anchored to the selected beach.
