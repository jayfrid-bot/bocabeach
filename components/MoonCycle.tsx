...
/**
 * The full lunar cycle with the current position marked: all eight phases in
 * order, the active one highlighted, and a marker on a progress track showing
 * exactly where tonight falls between new moon (0) and the next new moon.
 */
export function MoonCycle({ moon }: { moon: NonNullable<SunData["moonPhase"]> }) {
      ...
      <div className="mt-2 flex justify-between" aria-hidden>
        {PHASES.map((p, i) => (
          <span
            ...
            className={
              i === activeIdx
                ? "scale-125 rounded-full bg-slate-700/60 px-1 text-base ring-1 ring-amber-400/60"
                : "px-1 text-base opacity-40"
            }
          >
            {p}
          </span>
