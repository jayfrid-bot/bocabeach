...
// Fixed, generic wording for every non-ok path (Codex LOW): the note is part
// of every public conditions response, so it must never leak an env var
// name, a key-rejection reason, or anything else internal-diagnostic.
const UNAVAILABLE_NOTE = "Traffic data isn't available";
...
export async function fetchTraffic(loc: Location): Promise<Wrapped<TrafficData>> {
  ...
  try {
    ...
    return {
      ...
      attribution: ATTRIBUTION,
      data,
      note: data.level === "unknown" ? UNAVAILABLE_NOTE : undefined,
    };
  } catch {
