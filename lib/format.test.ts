...
describe("seaState", () => {
  it("maps wave height to the plain-English ladder", () => {
    ...
    expect(seaState(2.4).label).toBe("Light chop");
    expect(seaState(3.5).label).toBe("Choppy");
    expect(seaState(5).label).toBe("Really choppy");
    expect(seaState(7).label).toBe("Big waves");
  ...
  it("treats band edges and bad input sanely", () => {
    ...
    expect(seaState(3).label).toBe("Choppy");
  });
