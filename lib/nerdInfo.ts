...
const nerdBuilders: Record<NerdKey, (ctx: NerdContext) => NerdInfo> = {
  ...
  sandTemp: ({ d, snap }) => {
    ...
    return {
      ...
      explainer:
        "Nobody actually measures beach sand, so we estimate it: start from the weather model's ground-surface temperature and add a dry-sand solar boost, because loose dry sand traps the sun's heat and runs far hotter than generic modeled ground. The whole model is calibrated against 20+ real infrared-thermometer readings taken on Boca sand across 7 field sessions in June–July 2026 — midday estimates land within about ±2°F of what the IR gun actually reads. The cloud input isn't a forecast: it's NOAA's GOES-19 satellite Clear Sky Mask (2 km pixels, refreshed roughly every 5 minutes), and we sample it ALONG THE SUN'S BEAM PATH — stepping boxes toward the sun at h ÷ tan(elevation) — because when the sun is low the cloud that actually shades the sand sits kilometres away, not overhead. A thermal-memory afternoon decay (discovered in a 15-reading field session where the same ~42° sun elevation gave a +33°F boost at 9:54 AM but only +1°F at 5 PM) tapers the boost from 1.4 h to 4.4 h past solar noon, and the wetter, firmer sand down by the surf runs about 0.65× the dune boost — roughly 10°F cooler, measured.",
      formula:
